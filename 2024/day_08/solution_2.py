def get_antinode_locations(location_a, location_b, map_height, map_width):
    r = set()
    antinode_1_x, antinode_1_y = location_a[1], location_a[0]
    while (0 <= antinode_1_x < map_width and 0 <= antinode_1_y < map_height):
        if location_a[0] > location_b[0]:
            antinode_1_y = antinode_1_y + abs(location_b[0] - location_a[0])
        else:
            antinode_1_y = antinode_1_y - abs(location_b[0] - location_a[0])
        if location_a[1] > location_b[1]:
            antinode_1_x = antinode_1_x + abs(location_b[1] - location_a[1])
        else:
            antinode_1_x = antinode_1_x - abs(location_b[1] - location_a[1])
        if 0 <= antinode_1_x < map_width and 0 <= antinode_1_y < map_height:
            r.add((antinode_1_y, antinode_1_x))

    antinode_2_x, antinode_2_y = location_b[1], location_b[0]
    while (0 <= antinode_2_x < map_width and 0 <= antinode_2_y < map_height):
        if location_a[0] > location_b[0]:
            antinode_2_y = antinode_2_y - abs(location_b[0] - location_a[0])
        else:
            antinode_2_y = antinode_2_y + abs(location_b[0] - location_a[0])

        if location_a[1] > location_b[1]:
            antinode_2_x = antinode_2_x - abs(location_b[1] - location_a[1])
        else:
            antinode_2_x = antinode_2_x + abs(location_b[1] - location_a[1])
